...
- [ ] هماهنگی با هاست سی‌پنل برای اجرای برنامه Node.js روی پورت 8729 و دسترسی به SSH/FTP
...
- [x] ایجاد `.env.example` با متغیرهای پایه (PORT=8729، NEXT_PUBLIC_SITE_URL، MONGODB_URI، SMS Provider)
- [x] پیاده‌سازی `server.js` برای اجرای Next.js روی پورت 8729 با `next start`
...
- [ ] استقرار روی سی‌پنل با `node server.js` و پورت 8729
- [ ] اتصال دامنه `https://samtamir.ir/` به برنامه Node.js (تنظیمات ریورس پراکسی یا Application Manager)
...
## فاز 13 — مهاجرت دامنه و برند به SamTamir / `https://samtamir.ir`
- [x] به‌روزرسانی `env.example` برای تنظیم `NEXT_PUBLIC_SITE_URL=https://samtamir.ir` و مقادیر PWA (`NEXT_PUBLIC_PWA_NAME`, `NEXT_PUBLIC_PWA_SHORT_NAME`, `NEXT_PUBLIC_PWA_DESCRIPTION`) و پیام OTP به برند «سام‌تعمیر».
- [x] به‌روزرسانی تنظیمات PWA و متادیتا:
  - [x] تغییر نام و توضیحات PWA در `public/manifest.json` به «سام‌تعمیر / SamTamir CRM».
  - [x] تغییر عنوان و توضیحات متای اصلی در `app/layout.tsx` به برند سام‌تعمیر.
  - [x] تنظیم `apple-mobile-web-app-title` در `components/pwa/pwa-head.tsx` به «سام‌تعمیر».
  - [x] به‌روزرسانی متن دکمه نصب PWA در `components/pwa/pwa-installer.tsx` به «نصب اپلیکیشن سام‌تعمیر».
  - [x] به‌روزرسانی نام cache در `public/sw.js` به `samtamir-v1`.
- [x] به‌روزرسانی برند در UI اپ اصلی:
  - [x] تغییر برند هدر سایدبار در `components/layout/app-shell.tsx` به «SamTamir CRM / سام‌تعمیر».
  - [x] تغییر متن صفحه اصلی در `app/page.tsx` به «سام‌تعمیر | SamTamir CRM» و توضیح جدید.
  - [x] تغییر متون ورود OTP در `components/auth/otp-card.tsx` به «ورود به سام‌تعمیر» و «شرایط و قوانین سام‌تعمیر».
- [x] به‌روزرسانی CORS برای دامنه جدید در `next.config.ts` تا از `NEXT_PUBLIC_SITE_URL` یا `https://samtamir.ir` استفاده کند.
- [ ] به‌روزرسانی تمام مراجع دامنه قدیمی در مستندات و راهنمای تست:
  - [ ] جایگزینی `https://tamirban1.ir` با `https://samtamir.ir` در پوشه `راهنمای-تست` و فایل‌های `docs/*` و `explaination.md` (به‌خصوص env نمونه و Endpointها).
  - [ ] بررسی اینکه نمونه‌های Postman و Flutter (`docs/flutter-implementation-guide.md`, `docs/flutter-preparation.md`) از دامنه جدید استفاده کنند.
- [ ] به‌روزرسانی نام پروژه در مستندات اصلی:
  - [ ] تغییر عنوان‌ها و توضیحات که به «تعمیربان / TamirBan» اشاره دارند به برند جدید «سام‌تعمیر / SamTamir» در فایل‌های کلیدی مثل `README.md`, `docs/README.md`, `explaination.md`, `docs/testing-strategy.md`.
- [ ] تست نهایی روی دامنه جدید `https://samtamir.ir` (Chrome دسکتاپ و موبایل):
  - [ ] تست صفحه اصلی، ورود OTP، داشبورد و ماژول‌های اصلی.
  - [ ] تست نصب PWA و اجرای آفلاین (بر اساس راهنمای `docs/pwa-guide.md`).
  - [ ] بررسی متا‌تگ‌ها و عنوان تب مرورگر برای نمایش درست برند سام‌تعمیر.
...
- [x] تولید و بازگرداندن JWT پس از تایید کد OTP صحیح و ذخیره نشست در دیتابیس
- [x] سنجش اتصال با این سناریو: ثبت شماره آزمایشی، ارسال کد OTP واقعی از طریق سرویس پیامکی، دریافت پاسخ موفق از API و ورود از طریق فرانت‌اند
...
- [x] **مرحله 3: اتصال به سرویس OTP**
  ...
  - [x] حذف بازگرداندن کد OTP از پاسخ API در همه محیط‌ها (Development و Production)
...
- [ ] ساخت Build نهایی (`next build`)
- [ ] استقرار روی سی‌پنل با `node server.js` و پورت 8729
- [ ] اتصال دامنه `https://samtamir.ir/` به برنامه Node.js
- [ ] تنظیم و تست SSL روی دامنه
...
## 🔧 به‌روزرسانی Pattern SMS — اولویت فوری

### وضعیت فعلی
- ✅ ارسال SMS با متن مستقیم پیاده‌سازی شده
- ✅ کد پترن جدید دریافت شده: `0wopnv45vvw7mss`
- ✅ **پیاده‌سازی Pattern API تکمیل شد**

### چک‌لیست به‌روزرسانی Pattern SMS
- [x] **مرحله 1: افزودن متغیر محیطی Pattern Code** ✅
  - [x] افزودن `TABAN_SMS_PATTERN_CODE=0wopnv45vvw7mss` به `.env.example` ✅
  - [ ] بررسی نام متغیر Pattern در API تابان (مثلاً `pattern_code` یا `patternCode`)
  - [ ] بررسی ساختار `pattern_values` در API تابان (مثلاً `{code: "1234"}` یا `{otp: "1234"}`)
- [ ] **مرحله 2: به‌روزرسانی کلاینت تابان (`lib/vendors/taban-sms.ts`)**
  - [ ] افزودن پارامتر اختیاری `patternCode` به interface `SendSmsParams`
  - [ ] افزودن پارامتر اختیاری `patternValues` به interface `SendSmsParams`
  - [ ] به‌روزرسانی تابع `sendSms` برای پشتیبانی از Pattern:
    - [ ] اگر `patternCode` موجود باشد، استفاده از Pattern API (بدون `message`)
    - [ ] ساختار body: `{pattern_code, pattern_values, sending_type, from_number, params: {recipients}}`
    - [ ] اگر `patternCode` موجود نباشد، استفاده از روش فعلی (با `message`)
  - [ ] بررسی مستندات API تابان برای ساختار دقیق Pattern
- [ ] **مرحله 3: به‌روزرسانی تابع `sendOtpSms`**
  - [ ] خواندن `TABAN_SMS_PATTERN_CODE` از متغیرهای محیطی
  - [ ] اگر Pattern Code موجود باشد:
    - [ ] استفاده از Pattern به جای متن مستقیم
    - [ ] تعیین نام متغیر Pattern (مثلاً `code` یا `otp`) بر اساس مستندات
    - [ ] ساخت `patternValues` با کد OTP
  - [ ] اگر Pattern Code موجود نباشد:
    - [ ] استفاده از روش فعلی (متن مستقیم)
- [ ] **مرحله 4: به‌روزرسانی مستندات**
  - [ ] به‌روزرسانی `taban-sms-integration.md` با اطلاعات Pattern
  - [ ] افزودن مثال استفاده از Pattern در مستندات
  - [ ] به‌روزرسانی `env.example` با توضیحات Pattern
- [ ] **مرحله 5: تست Pattern SMS**
  - [ ] تست ارسال OTP با Pattern Code
  - [ ] بررسی دریافت پیامک با متن Pattern
  - [ ] بررسی ثبت لاگ در دیتابیس
  - [ ] تست Fallback به روش قدیمی (اگر Pattern Code موجود نباشد)
  - [ ] تست خطا در صورت Pattern Code نامعتبر

### نکات مهم
- Pattern Code: `0wopnv45vvw7mss`
- باید ساختار متغیرهای Pattern را از مستندات API تابان بررسی کنیم (مثلاً `{code}` یا `{otp}`)
- باید backward compatibility حفظ شود (اگر Pattern Code موجود نباشد، از روش قدیمی استفاده شود)
