...
#### 1.1. تغییر وضعیت پرداخت در صفحه جزئیات (اولویت: بالا) — 0.5 روز
- [ ] **Backend — بررسی API موجود**
  - [x] API `PATCH /api/invoices/[id]/status` موجود است ✅
  - [x] RBAC محافظت شده با `invoices:write` ✅
- [ ] **Frontend — UI تغییر وضعیت**
  - [ ] افزودن دکمه "تغییر وضعیت" در `components/invoices/invoice-detail-view.tsx`
  - [ ] ایجاد Modal `InvoiceStatusChangeModal`:
    - [ ] نمایش وضعیت فعلی
    - [ ] Dropdown برای انتخاب وضعیت جدید (DRAFT, SENT, PAID, OVERDUE, CANCELLED)
    - [ ] فیلد اختیاری "تاریخ پرداخت" (اگر وضعیت PAID انتخاب شود)
    - [ ] فیلد اختیاری "شماره مرجع پرداخت"
    - [ ] دکمه‌های "تایید" و "انصراف"
  - [ ] اتصال Modal به API `PATCH /api/invoices/[id]/status`
  - [ ] به‌روزرسانی خودکار UI پس از تغییر وضعیت (revalidate)
  - [ ] Confirmation dialog برای تغییرات مهم (PAID, CANCELLED)
  - [ ] نمایش Toast notification برای موفقیت/خطا
  - [ ] استفاده از `ProtectedComponent` برای محدود کردن دسترسی (فقط `invoices:write`)

#### 1.2. ویرایش پیش‌فاکتور (اولویت: بالا) — 1 روز
- [ ] **Backend — بررسی API موجود**
  - [x] API `PATCH /api/invoices/[id]` موجود است ✅
  - [x] Service `updateInvoice` موجود است ✅
  - [x] RBAC محافظت شده با `invoices:write` ✅
- [ ] **Frontend — UI ویرایش**
  - [ ] ایجاد Modal `InvoiceEditModal`:
    - [ ] Pre-fill داده‌های موجود از `invoice` prop
    - [ ] انتخاب مشتری (SearchableSelect)
    - [ ] انتخاب بازاریاب (SearchableSelect)
    - [ ] انتخاب تاریخ سررسید (PersianDateTimePicker)
    - [ ] مدیریت آیتم‌ها:
      - [ ] نمایش لیست آیتم‌های موجود
      - [ ] دکمه "افزودن آیتم جدید"
      - [ ] دکمه "حذف" برای هر آیتم
      - [ ] امکان ویرایش هر آیتم (title, quantity, unitPrice, discount, taxRate)
    - [ ] محاسبه خودکار جمع کل (subtotal, discountTotal, taxTotal, grandTotal)
    - [ ] فیلد "یادداشت" (textarea)
    - [ ] دکمه‌های "ذخیره" و "انصراف"
  - [ ] اتصال Modal به API `PATCH /api/invoices/[id]`
  - [ ] Validation:
    - [ ] بررسی وجود حداقل یک آیتم
    - [ ] بررسی مقادیر مثبت برای quantity و unitPrice
    - [ ] بررسی محدوده discount و taxRate (0-100)
  - [ ] به‌روزرسانی خودکار UI پس از ویرایش (revalidate)
  - [ ] نمایش Toast notification برای موفقیت/خطا
  - [ ] استفاده از `ProtectedComponent` برای محدود کردن دسترسی (فقط `invoices:write`)

#### 1.3. حذف پیش‌فاکتور (اولویت: متوسط) — 0.5 روز
- [ ] **Backend — بررسی API موجود**
  - [x] API `DELETE /api/invoices/[id]` موجود است ✅
  - [x] Service `deleteInvoice` موجود است ✅
  - [x] RBAC محافظت شده با `invoices:delete` ✅
- [ ] **Frontend — UI حذف**
  - [ ] افزودن دکمه "حذف" در `components/invoices/invoice-detail-view.tsx`
  - [ ] استفاده از `ProtectedComponent` برای محدود کردن دسترسی (فقط `invoices:delete`)
  - [ ] ایجاد Confirmation Dialog:
    - [ ] نمایش شماره پیش‌فاکتور
    - [ ] نمایش مبلغ کل
    - [ ] هشدار درباره غیرقابل برگشت بودن عملیات
    - [ ] دکمه‌های "تایید حذف" و "انصراف"
  - [ ] اتصال Dialog به API `DELETE /api/invoices/[id]`
  - [ ] ریدایرکت به `/dashboard/invoices` پس از حذف موفق
  - [ ] نمایش Toast notification برای موفقیت/خطا
  - [ ] مدیریت خطا (مثلاً اگر پیش‌فاکتور پرداخت شده باشد)

#### 1.4. ایندکس‌های MongoDB (اولویت: متوسط) — 0.5 روز
- [ ] **ایجاد ایندکس‌ها**
  ...
  - [ ] ایندکس برای `dueAt` در collection `invoices`:
    ```javascript
    db.invoices.createIndex({ dueAt: 1 })
    ```
  - [ ] ایندکس ترکیبی برای جستجوهای پیشرفته:
    ```javascript
    db.invoices.createIndex({ customerId: 1, status: 1, dueAt: -1 })
    ```
  - [ ] ایندکس برای `marketerId` در collection `invoices`:
    ```javascript
    db.invoices.createIndex({ marketerId: 1 })
    ```
- [ ] **ایجاد Migration Script**
  - [ ] ایجاد فایل `scripts/migrations/add-invoice-indexes.js`
...
### 9.1. ایجاد Manifest.json (اولویت: فوری)
- [ ] ایجاد فایل `public/manifest.json`
- [ ] تنظیم `name`: "تعمیربان CRM"
- [ ] تنظیم `short_name`: "تعمیربان"
- [ ] تنظیم `description`: "سیستم مدیریت مشتریان و ویزیت‌ها"
- [ ] تنظیم `theme_color`: "#3b82f6" (رنگ primary)
- [ ] تنظیم `background_color`: "#ffffff"
- [ ] تنظیم `display`: "standalone"
- [ ] تنظیم `start_url`: "/dashboard"
- [ ] تنظیم `orientation`: "portrait" (برای موبایل)
- [ ] تعریف آیکون‌ها (192x192 و 512x512)
- [ ] اتصال manifest به `app/layout.tsx` با `<link rel="manifest" href="/manifest.json" />`
- [ ] افزودن meta tags برای iOS Safari:
  - [ ] `<meta name="apple-mobile-web-app-capable" content="yes" />`
  - [ ] `<meta name="apple-mobile-web-app-status-bar-style" content="default" />`
  - [ ] `<meta name="apple-mobile-web-app-title" content="تعمیربان" />`

### 9.2. ایجاد آیکون‌های PWA (اولویت: فوری)
- [ ] طراحی یا تهیه آیکون اصلی (حداقل 512x512 پیکسل)
- [ ] ایجاد آیکون 192x192 پیکسل (برای Android)
- [ ] ایجاد آیکون 512x512 پیکسل (برای Android)
- [ ] ایجاد آیکون 180x180 پیکسل (برای iOS)
- [ ] قرار دادن در `public/icons/` یا `public/`
- [ ] تعریف در manifest.json:
  ...
  ```
- [ ] افزودن `<link rel="apple-touch-icon" href="/icons/icon-180.png" />` در `app/layout.tsx`

### 9.3. پیاده‌سازی Service Worker (اولویت: متوسط)
- [ ] ایجاد فایل `public/sw.js` یا استفاده از `next-pwa`
- [ ] نصب `next-pwa` (اختیاری) یا پیاده‌سازی دستی
- [ ] Cache کردن فایل‌های استاتیک:
  - [ ] CSS files
  - [ ] JS files
...
- [ ] Offline support:
  - [ ] ایجاد صفحه Offline (`public/offline.html`)
  - [ ] نمایش پیام مناسب در صورت عدم اتصال
- [ ] Background sync (اختیاری):
  - [ ] Sync کردن درخواست‌های ناموفق API
...
### 9.4. بهینه‌سازی برای PWA (اولویت: متوسط)
- [ ] بهبود Performance:
  - [ ] Lazy loading برای صفحات
  - [ ] Code splitting
  - [ ] Image optimization
...
### 9.6. مستندسازی PWA
- [ ] ایجاد `docs/pwa-guide.md`
- [ ] مستندسازی مراحل نصب
- [ ] مستندسازی قابلیت‌های PWA
...
### 10.4. صفحه مدیریت نقش‌ها (اولویت بالا) — 1 روز
- [ ] **Backend — API Endpoints**
  - [ ] ایجاد `GET /api/settings/roles` برای دریافت لیست نقش‌ها و permissions
  - [ ] ایجاد `POST /api/settings/roles` برای ایجاد نقش جدید (فقط SUPER_ADMIN)
  - [ ] ایجاد `PATCH /api/settings/roles/[id]` برای ویرایش permissions (فقط SUPER_ADMIN)
  - [ ] ایجاد `GET /api/settings/roles/[id]/users` برای نمایش کاربران هر نقش
  - [ ] محافظت از تمام endpoints با `requireRole("SUPER_ADMIN")`
- [ ] **Frontend — UI کامل**
  - [ ] ایجاد صفحه `/dashboard/settings/roles`
  - [ ] نمایش لیست نقش‌ها با permissions هر نقش
  - [ ] Modal برای ایجاد نقش جدید
  - [ ] Modal برای ویرایش permissions نقش
  - [ ] نمایش تعداد کاربران هر نقش
  - [ ] دکمه "مشاهده کاربران" برای هر نقش
  - [ ] استفاده از `ProtectedComponent` برای محدود کردن دسترسی
- [ ] **تست**
  - [ ] تست دسترسی فقط برای SUPER_ADMIN
  - [ ] تست ایجاد و ویرایش نقش
  - [ ] تست نمایش کاربران هر نقش
...
| فاز | مدت زمان | اولویت | وضعیت |
|-----|----------|--------|-------|
| **فاز 10: تکمیل RBAC** | **3-4 روز** | **بالا** | ✅ **90% تکمیل** (10.1 ✅, 10.2 ✅, 10.3 ✅, 10.4 ⏳) |
| **فاز 9: PWA** | **2-3 روز** | **بالا** | ⏳ در انتظار |
| فاز 1: تکمیل Invoices | 2-3 روز | بالا | ⏳ در انتظار |
| فاز 2: SMS Center | 3-4 روز | متوسط | ⏳ در انتظار |
| فاز 3: Reports | 2-3 روز | متوسط | ⏳ در انتظار |
...
> 1. **فاز 10.4 (صفحه مدیریت نقش‌ها)** — تکمیل RBAC (1 روز)
> 2. **فاز 9 (PWA)** — برای تبدیل به اپ موبایل (2-3 روز)
> 3. **فاز 1 (تکمیل Invoices)** — برای MVP (2-3 روز)
> 4. **فاز 6 (تست‌ها)** — برای کیفیت (3-4 روز)
