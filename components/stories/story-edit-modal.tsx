...
import { updateStoryAction } from "@/app/dashboard/stories/actions";
...
export function StoryEditModal({ story, isOpen, onClose, onSuccess }: StoryEditModalProps) {
  const [state, formAction] = useActionState(updateStoryAction, {
    success: false,
    message: null,
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-2xl rounded-3xl border-2 border-slate-300 bg-white shadow-xl max-h-[95vh] overflow-y-auto">
        <div className="sticky top-0 flex items-center justify-between border-b-2 border-slate-200 bg-white px-6 py-4">
          <h2 className="text-lg font-semibold text-slate-800">ویرایش استوری</h2>
          <button
            onClick={onClose}
